...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for i in range(0, number_of_groups):
            ...
            if print_coords: print(str(junk_point.location[1]) + "\t" + str(junk_point.location[0]) + "\t" + "circle5")
            if random.random() > (1-probability_of_multiple_junk_point):
                ...
                for l in range(0, amount):
                    ...
                    if print_coords: print(str(neighbour.location[1]) + "\t" + str(neighbour.location[0]) + "\t" + "cross5")
            else:
